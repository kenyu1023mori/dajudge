...
def predict_score(input_text, model, tokenizer, mecab):
    ...
    with torch.no_grad():
        ...
        prediction = torch.sigmoid(torch.tensor(prediction)).item()  # Apply sigmoid to the prediction
        predicted_class = 1 if prediction >= 0.5 else 0  # Binary classification threshold
        print(f"Predicted Class: {'Funny' if predicted_class == 1 else 'Not Funny'}")
